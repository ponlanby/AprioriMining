...
public class improved_weighted_apriori {
...
//		frequentSetMap.putAll(stepFrequentSetMap);//生成频繁1项集
		...
		Map<String, Float> candidateSetMap = new TreeMap<>();
...
//					float weight = Float.parseFloat(data.substring(data.lastIndexOf(ITEM_SPLIT)+1, data.length()));
					for (String string : strings) {
						countFLG++;
						if (data.indexOf(string + ITEM_SPLIT) == -1) {
							flag = false;
					...
					if (flag)
						candidateSetMap.put(candidate,
								candidateSetMap.get(candidate) + Float.parseFloat(data.substring(data.lastIndexOf(ITEM_SPLIT)+1, data.length())));
				}
			}
...
//		}
		...
		improved_weighted_apriori apriori2 = new improved_weighted_apriori();
...
//		System.out.println();
	...
	public static void writeToTxt(Map<String, Float> frequentSetMap){
		Set<String> keySet = frequentSetMap.keySet();
...
//			fw.newLine();
	...
	public static void saveCandidate(Map<String, Float> candidateSetMap){
